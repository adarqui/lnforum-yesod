...
getThreadPostPacksM m_sp user_id = do
  ...
  case (lookupSpMay m_sp spThreadId
       ,lookupSpMay m_sp spThreadPostId
       ,lookupSpMay m_sp spThreadPostIds) of

    (Just thread_id, _, _) -> getThreadPostPacks_ByThreadIdM m_sp user_id thread_id

    (_, Just post_id, _)   -> getThreadPostPacks_ByThreadPostIdM m_sp user_id post_id

    (_, _, Just post_ids)  -> getThreadPostPacks_ByThreadPostIdsM m_sp user_id post_ids
...
getThreadPostPack_ByThreadPostM m_sp user_id thread_post@(Entity thread_post_id ThreadPost{..}) = do
  ...
  rehtie lr leftA $
    \(thread_post_user, thread_post_stat, m_thread_post_like, user_perms_by_thread_post, m_board, m_thread, m_offset, m_posts) -> do
